...
// Book name lookup table (All 66 books)
const bookNames = {
...
// Function to render the Bible
function renderBible() {
    const container = document.getElementById("bibleContainer");
    if (!container) {
        console.error("Error: Could not find #bibleContainer in HTML.");
        return;
    }

    for (const bookNum in kjv25) {
        const bookName = bookNames[bookNum] || `Book ${bookNum}`;
        const chapters = kjv25[bookNum];
        ...
        for (const chapterNum in chapters) {
            ...
            verses.forEach((verseText, verseNum) => {
                ...
                verseElement.textContent = `${bookName} ${chapterNum}:${verseNum + 1} ${verseText}`;
...
// Load the Bible on page load
document.addEventListener("DOMContentLoaded", renderBible);